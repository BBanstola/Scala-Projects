...
def der (c: Char, r: Rexp) : Rexp = r match {
  case ZERO => ZERO
  case ONE => ZERO
  case CHAR(other) => if (c == other) ONE else ZERO
  case ALT(r1, r2) => ALT(der(c, r1), der(c, r2))
  case SEQ(r1, r2) if (nullable(r1)) => ALT(SEQ(der(c, r1), r2), der(c, r2))
  case SEQ(r1, r2) => SEQ(der(c, r1), r2)
...
def simp(r: Rexp) : Rexp = r match{
  ...
  case ALT(r1 , r2) => {
    (simp(r1), simp(r2)) match{
      case (regx , ZERO) => simp(regx)
      case (ZERO, regx) => simp(regx)
      case (a, b) if(a == b) => simp(a)
  ...
  case SEQ(r1, r2) => {
    (simp(r1), simp(r2)) match{
      case (_ , ZERO) => ZERO
      case (ZERO, _) => ZERO
      case (regx, ONE) => simp(regx)
      case (ONE, regx) => simp(regx)
      case (a, b) => SEQ(simp(r1), simp(r2))
    }
  }
...
//def ders (s: List[Char], r: Rexp) : Rexp = ... 
  ...
  def matcher(r: Rexp, s: String): Boolean = if (nullable(ders(s.toList, r))) true else false
